...
- Produces:
  - `ConfigError::Inconsistent { problems: Vec<String> }` — **rendered strings, not `Problem`**; see the note below
...
```rust
    ...
    /// Hide the keymap warnings for this run. Wins over `--warnings` and over
    /// `RECON_WARNINGS` when both are given.
    //
    // Deliberately no `conflicts_with`: clap counts an env-provided value as
    // present, so declaring a conflict here made exporting `RECON_WARNINGS`
    // stop `--no-warnings` from starting recon at all — the durable setting
    // vetoing the per-run flag meant to override it. The resolver
    // short-circuits on this flag instead, so giving both is defined rather
    // than refused. Keep the rationale in a plain comment: doc comments render
    // into `--help`, and `long_help_carries_no_maintainer_rationale` bans
    // maintainer prose there.
...
/// The budget must count rendered rows, not warnings admitted. One fixture
/// covers all three cases the other tests miss: more than one warning, text
/// long enough to wrap, and a box too short to hold it all.
#[test]
...
/// The other side of the same boundary: given room, every warning renders and
/// nothing is reported cut.
///
/// The `filters.down` assertion is the one doing the regression work. Under
/// the broken arithmetic the box was sized from the *count* of warnings, so
/// the third never appeared however much room the terminal had. The
/// "no (N more)" assertion is a correctness check on the title, not a guard —
/// `cut` computes 0 under both the broken and fixed arithmetic here.
#[test]
...
```rust
/// `app_over`, with a `[keymap]` that costs `nav.down` its `j` — the smallest
/// config that produces a warning.
...
/// `app_over` has 58 callers, and `app_over_files` beside it already repeats
/// this shape rather than sharing a body — so repeating it once more is the
/// convention here, and rewriting `app_over` would be 58 call sites of churn
/// for nothing.
...
Every test gets its own fixture name. They share a parent directory, and this
repository has already been bitten once by two fixture names that collided on a
...
```rust
    ...
    fn render_keymap_warnings(&self, area: Rect, buf: &mut Buffer) {
        ...
        for warning in &self.keymap_warnings {
            // The bullet is measured with the text, not added after it.
            ...
            // and the one before.
            let cost = if lines.is_empty() { wrapped } else { wrapped + 1 };
        ...
        // the borders. `rows + 4` is exactly the content's real requirement.
...
**Files:**
- Modify: `README.md` (the `### Configuring the keymap` section, which begins at `README.md:451`)

**Interfaces:**
- Consumes: everything above.
- Produces: nothing.

**Read this before you start — two things changed under this task.**

**(a) The `--print-keymap` rewrite is already done.** Task 7 rewrote that
paragraph as part of its own commit, because the machine-checked doc tests
would not let the code and the README disagree across a commit boundary. The
false sentence saying `--print-keymap` prints the defaults is already gone, and
`README.md:466-487` now describes the effective map, the annotation comments,
and `--print-keymap defaults`, with a worked `console` example. **Do not
rewrite it, and do not add a second paragraph saying the same thing.** Read it
first so that what you add agrees with it in wording and in fact.

**(b) Add flat prose, not a `####` subsection.** The plan originally specified
a `#### When two actions want one key` heading. Do not add one. `###
Configuring the keymap` at `README.md:451` runs all the way to the next `###`
at `README.md:1017`, so every line of the filter documentation already sits
underneath it. A `####` inserted at the end of the keymap prose would capture
all of that filter text as its content. The section's own habit is flat prose
anyway — the key-spelling rules and the reserved-key paragraph carry no
headings.

What is genuinely missing, and what this task owes: the consistency rules, and
the warnings switch. Neither appears anywhere in the README today —
`--no-warnings` and `RECON_WARNINGS` occur only inside the generated usage
block at `README.md:250-251`, with no prose explaining them.

- [ ] **Step 1: Add the consistency rules and the switch**

The keymap prose ends at `README.md:514` with the reserved-key paragraph, whose
last line is:
...
Insert a blank line after it and then the text below, so it lands between that
paragraph and the filter prose that begins `Filters colour the lines they
match`:
...
- [ ] **Step 2: Check every claim against the binary**
...
An entry replaces an action's keys, so it can take a key from another
action. Say what recon refuses, what it warns about, and how to turn the
warnings off."
```
...
**3. Type consistency.** `Problem` is the enum from Task 2 everywhere, and it stays `pub(crate)` and never reaches a public signature: `ConfigError::Inconsistent` and `Config::keymap_warnings` both carry `Vec<String>`, rendered through `Problem`'s `Display` inside `build_keymap`, because `ConfigError` is public API and a `pub(crate)` type in one of its variants is E0446. `Report` fields are `errors`, `warnings`, `evict` in all four tasks that touch them. `check(built, written)` keeps one signature. `print_keymap(keymap, defaults)` is the same in Tasks 7 and 8. `build_keymap` returns `(Keymap, Vec<String>)` from Task 4 on, and Tasks 6 and 7 both consume that pair. One inconsistency found and fixed: `Keymap::new`'s return type changed in Task 5 while Task 4 already called it, so Task 4's code would have broken two tasks later. The signature change moved into Task 4 (Step 7b), where `build_keymap` is already being rewritten and the behaviour stays identical; Task 5 now only adds the gate. `Keymap::new` therefore returns `Result<(Keymap, Vec<String>), ConfigError>` from Task 4 onward, and every task after it destructures the pair.

...
mod tests {
    ...
    fn two_written_lines_on_one_key_in_one_scope_are_an_error() {
        let report = report(&[("global.quit", &["x"]), ("global.reload", &["x"])]);
    ...
    fn a_written_line_takes_a_key_from_another_actions_default() {
        // 'o' is nav.open's default. nav.up asks for it.
        let report = report(&[("nav.up", &["o"])]);
        ...
        assert_eq!(
            ...
            vec![Problem::Displaced {
                scope: Scope::Nav,
                key: "o".to_string(),
                winner: ActionId::NavUp,
                loser: ActionId::NavOpen,
                remaining: vec![],
            }],
            "nav.open has only 'o', so it is left with no key at all"
        );
        assert_eq!(
            report.evict,
            vec![(Scope::Nav, "o".to_string(), ActionId::NavOpen)],
...
```rust
    ...
    fn a_pane_line_under_a_written_global_binding_is_also_an_error() {
        let report = report(&[("global.reload", &["z"]), ("nav.up", &["z"])]);
    ...
    fn a_modal_scope_never_crosses_with_global() {
        // 'Enter' is prompt.commit in the prompt scope and picker.choose in
        // the picker scope. A global line binding it is not a collision:
        // while either modal scope is open it owns the whole keyboard.
        let report = report(&[("global.reload", &["Enter"])]);

...
- [ ] **Step 7c: Remove the two temporary `dead_code` attributes**

Tasks 2 and 3 created the checker two tasks before anything called it. Every item in it is `pub(crate)`, so rustc's dead-code analysis had no root to reach them from and `clippy -D warnings` failed the lib target. Two attributes bridged that gap, each carrying a comment saying it goes when the checker is wired in. **This task is that wiring, so both come out now:**
...
Delete the attributes and their comments, then run `cargo clippy --workspace --all-targets -- -D warnings`. It must pass with them gone. If any item is still reported dead, that item genuinely has no caller and the finding is real — report it rather than restoring the attribute.
